  ...
  return (
    <div className="bg-card border rounded-lg p-4 shadow-lg min-w-[260px]">
      ...
      <div className="space-y-2 text-sm">
        ...
        {typeof data.deltaPercentuale === "number" && (
          <div className="flex justify-between items-center">
            <span className="text-muted-foreground">Delta:</span>
            <Badge
              variant={data.value < 0 ? "default" : "destructive"}
              className={data.value < 0 ? "bg-green-600" : ""}
            >
...
export function WaterfallComposizioneDelta({
  ...
  const waterfallData = useMemo(() => {
    ...
    // Ordina le categorie per contributo (dal maggiore risparmio al maggiore extra-costo)
    ...
    sortedData.forEach((item) => {
      ...
      chartData.push({
        ...
        delta: item.delta,
        deltaPercentuale: item.deltaPercentuale,
      });
    });

    // Punto finale (Offerta)
    chartData.push({
      name: "Offerta Migliore",
  ...
  const stats = useMemo(() => {
    ...
    const deltaPercentualeComplessivo =
      ((deltaComplessivo / importoProgettoTotale) * 100);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <InfoTooltip content="Mostra come si compone il delta totale tra progetto e offerta migliore, categoria per categoria. Evidenzia dove si concentrano risparmi ed extra-costi." />
          </CardTitle>
          <CardDescription>
            Analisi dettagliata delle variazioni per categoria WBS
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center py-12 text-muted-foreground">
            Dati insufficienti per mostrare la composizione del delta.
          </div>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Scomposizione del delta tra importo progetto e offerta migliore
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 rounded-lg border bg-muted/30">
            ...
            <p className="text-2xl font-bold text-muted-foreground">
              {formatEuro(importoProgettoTotale)}
            </p>
          </div>

          <div className="p-4 rounded-lg border bg-primary/5 border-primary/20">
            <p className="text-xs text-muted-foreground mb-1">Offerta Migliore</p>
            <p className="text-2xl font-bold text-primary">
              {formatEuro(importoOffertaTotale)}
            </p>
          </div>

          <div
            className={`p-4 rounded-lg border ${
              stats.deltaComplessivo < 0
              ...
              <div>
                <p
                  className={`text-2xl font-bold ${
                    stats.deltaComplessivo < 0 ? "text-green-600" : "text-destructive"
                  }`}
                >
                ...
                >
                  {stats.deltaPercentualeComplessivo > 0 ? "+" : ""}
                  {stats.deltaPercentualeComplessivo.toFixed(1)}%
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Breakdown Risparmi/Extra-costi */}
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center pb-2 border-b border-green-200 dark:border-green-900">
                ...
                <span className="font-bold text-green-600 text-lg">
                  {formatEuro(-stats.risparmi.totale)}
                </span>
              </div>
              <p className="text-xs text-muted-foreground mb-3">
                {stats.risparmi.count} categorie con prezzi inferiori al progetto
              </p>
                ...
                <div key={idx} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-mono font-semibold text-green-600">
                      {formatEuro(item.delta)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>Delta: {item.deltaPercentuale.toFixed(1)}%</span>
                  </div>
                  {idx < stats.risparmi.top.length - 1 && (
                    <div className="h-px bg-green-200 dark:bg-green-900 my-2" />
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center pb-2 border-b border-destructive/20">
                ...
                <span className="font-bold text-destructive text-lg">
                  +{formatEuro(stats.extraCosti.totale)}
                </span>
              </div>
              <p className="text-xs text-muted-foreground mb-3">
                {stats.extraCosti.count} categorie con prezzi superiori al progetto
              </p>
              {stats.extraCosti.top.length > 0 ? (
                stats.extraCosti.top.map((item, idx) => (
                  <div key={idx} className="space-y-1">
                    ...
                    {idx < stats.extraCosti.top.length - 1 && (
                      <div className="h-px bg-destructive/20 my-2" />
                    )}
                  </div>
                ))
              ) : (
                <p className="text-sm text-muted-foreground text-center py-4">
                  Nessun extra-costo rilevato
                </p>
              )}
            </div>
          </div>
        ...
        <div className="p-4 bg-muted/30 rounded-lg border border-border">
          ...
          <ul className="text-sm space-y-1 text-muted-foreground">
            <li>
              • Il <span className="font-semibold text-foreground">{
                ((Math.abs(stats.risparmi.totale) / (Math.abs(stats.risparmi.totale) + stats.extraCosti.totale)) * 100).toFixed(0)
              }%</span> del delta deriva da risparmi, il resto da extra-costi
            </li>
            {stats.risparmi.top.length > 0 && (
              <li>
                • Maggiore risparmio:{" "}
                <span className="font-semibold text-green-600">
                  {stats.risparmi.top[0].categoria}
                </span>
                {" "}({formatEuro(stats.risparmi.top[0].delta)}, {stats.risparmi.top[0].deltaPercentuale.toFixed(1)}%)
              </li>
            )}
            {stats.extraCosti.top.length > 0 && (
              <li>
                • Maggiore extra-costo:{" "}
                <span className="font-semibold text-destructive">
                  {stats.extraCosti.top[0].categoria}
                </span>
                {" "}(+{formatEuro(stats.extraCosti.top[0].delta)}, +{stats.extraCosti.top[0].deltaPercentuale.toFixed(1)}%)
              </li>
            )}
            <li className="pt-2 border-t mt-2">
              • Concentra le negoziazioni sulle categorie rosse per massimizzare i risparmi
            </li>
          </ul>
        </div>
      </CardContent>
