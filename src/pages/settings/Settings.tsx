...
import type { ApiCommessa, ApiNlpModelOption, ApiSettingsResponse } from "@/types/api";
...
const settingsSchema = z.object({
  ...
  nlp_batch_size: z.coerce
    .number()
    .min(4, "Il batch non può essere inferiore a 4")
    .max(256, "Il batch non può superare 256"),
  nlp_max_length: z.coerce
    .number()
    .min(64, "La lunghezza minima deve essere almeno 64 token")
    .max(2048, "La lunghezza massima non può superare 2048 token"),
});
...
export default function Settings() {
  ...
  const [settingsMeta, setSettingsMeta] = useState<ApiSettingsResponse["settings"] | null>(null);
  ...
  const activeEmbeddingModel = availableModels.find((model) => model.id === embeddingModelId);
    ...
    try {
      if (form.formState.isDirty) {
        ...
        toast({
          ...
          description: "Il modello selezionato è stato salvato. Avvio la rigenerazione.",
        });
      }
  ...
  const handleNormalizeImprese = async () => {
    try {
      ...
      const commessaId =
        selectedCommessaImpreseId === "all" ? undefined : Number(selectedCommessaImpreseId);
    ...
    if (saved) {
      toast({
        ...
        description: `Il modello ${
          selectedModel?.label ?? form.getValues().nlp_model_id
        } è pronto all'uso.`,
      });
    }
  };
  ...
  const handleRegenerateProperties = async () => {
    try {
      ...
      const commessaId =
        selectedCommessaPropsId === "all" ? undefined : Number(selectedCommessaPropsId);
  ...
  return (
    <PageShell
      title="Impostazioni"
      description="Configura i parametri per l’analisi dei computi, i modelli NLP e gli strumenti avanzati."
      bodyClassName="flex flex-col gap-4 bg-muted/30"
    >
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className="flex-1 space-y-4 px-4 py-4 sm:px-6 lg:px-8"
        >
          <div className="flex items-center justify-between gap-3">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Le modifiche vengono applicate globalmente a tutte le commesse.
              </p>
            </div>
          ...
          <Tabs defaultValue="generale" className="space-y-4">
            <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4">
              ...
              <TabsTrigger value="modelli">Modelli & embedding</TabsTrigger>
              <TabsTrigger value="imprese">Imprese</TabsTrigger>
              <TabsTrigger value="proprieta">Proprietà</TabsTrigger>
            </TabsList>
              ...
              <Card className="rounded-2xl border border-border/60 bg-card shadow-md">
                <CardHeader>
                  <CardTitle className="text-lg font-semibold">
                    Parametri Delta Critico
                  </CardTitle>
                  <CardDescription>
                    Definisci le soglie per identificare automaticamente le voci critiche nei
                    confronti tra progetto e offerte delle imprese.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Prezzi gara bassi se il delta è minore di questo valore (valore
                            negativo).
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Prezzi gara alti se il delta è maggiore di questo valore (valore
                            positivo).
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            CME considerato alto se il delta percentuale è maggiore di questo
                            valore.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            CME considerato basso se il delta percentuale è minore di questo
                            valore.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                ...
                <CardContent className="space-y-4 text-sm">
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      I valori di delta minimo e massimo vengono usati per identificare voci con
                      scarti monetari significativi rispetto al progetto, indipendentemente dalla
                      percentuale.
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      Le percentuali vengono usate per classificare le voci in base allo scarto
                      relativo, identificando automaticamente le criticità nell&apos;analisi dei
                      ritorni di gara.
                    </p>
                  </div>
                  <div>
                    <h4 className="mb-2 text-sm font-semibold">Livelli di criticità</h4>
            ...
            {/* TAB MODELLI & EMBEDDING */}
            <TabsContent value="modelli" className="space-y-4">
              <Card className="rounded-2xl border border-border/60 bg-card shadow-md">
                <CardHeader>
                  <CardTitle className="text-lg font-semibold">
                    Ricerca semantica e modelli AI
                  </CardTitle>
                  <CardDescription>
                    Configura e controlla lo stato dei modelli utilizzati per gli embedding.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/5 to-primary/10 p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Gli embedding nel database sono stati calcolati con questo modello.
                        </p>
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                      <div>
                        ...
                        <p className="font-semibold">
                          {activeEmbeddingModel?.label ??
                            "Rigenera per applicare il nuovo modello"}
                        </p>
                        <p className="font-mono text-xs text-muted-foreground">
                          {activeEmbeddingModel?.id ?? embeddingModelId ?? "N/D"}
                        </p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Lingue supportate</p>
                        <p className="font-semibold">
                          {activeEmbeddingModel?.languages ?? "N/D"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                                ...
                                <SelectItem key={model.id} value={model.id}>
                                  <div className="flex flex-col items-start">
                                    ...
                                    <span className="text-xs text-muted-foreground">
                                      {model.speed} · {model.dimension} dim
                                    </span>
                                  </div>
                                </SelectItem>
                              ))}
                          ...
                          <FormDescription>
                            Modelli più grandi offrono risultati migliori ma richiedono più
                            risorse.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    {selectedModel && (
                      <div className="rounded-xl border border-dashed border-border/70 p-3 text-xs text-muted-foreground">
                        <p className="font-semibold text-foreground">{selectedModel.label}</p>
                        <p className="font-mono">{selectedModel.id}</p>
                        <p>
                          {selectedModel.dimension} dim · {selectedModel.speed} ·{" "}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              Elementi processati in parallelo durante il calcolo degli embedding.
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              Lunghezza massima del testo passato al modello NLP.
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
                  ...
                  {showRegenerationWarning && (
                    <Alert variant="destructive">
                      ...
                      <AlertDescription>
                        Gli embedding salvati non sono allineati con il modello selezionato.
                        Rigenera per ottenere risultati coerenti.
                      </AlertDescription>
                    </Alert>
                  )}
                  ...
                  <div className="border-t pt-4">
                    ...
                    <p className="mb-4 text-sm text-muted-foreground">
                      Esegui dopo aver cambiato il modello o se gli embedding non sono stati
                      calcolati durante l&apos;importazione.
                    </p>
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <div className="flex-1">
                        <Select
                          value={selectedCommessaId}
                          onValueChange={setSelectedCommessaId}
            ...
            <TabsContent value="imprese" className="space-y-4">
              <Card className="rounded-2xl border border-border/60 bg-card shadow-md">
                <CardHeader>
                  <CardTitle className="text-lg font-semibold">
                    Normalizza imprese (ritorni)
                  </CardTitle>
                  <CardDescription>
                    Rimuove suffissi duplicati (es. “(2)”) e riallinea offerte e computi
                    all&apos;impresa normalizzata.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <div className="flex-1">
                      <Select
                        value={selectedCommessaImpreseId}
                        onValueChange={setSelectedCommessaImpreseId}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Usa questa azione dopo import multipli per eliminare etichette duplicate e
                    allineare i filtri per impresa.
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            {/* TAB PROPRIETÀ */}
            <TabsContent value="proprieta" className="space-y-4">
              <Card className="rounded-2xl border border-border/60 bg-card shadow-md">
                <CardHeader>
                  <CardTitle className="text-lg font-semibold">
                    Ricalcolo proprietà estratte
                  </CardTitle>
                  <CardDescription>
                    Rielabora le voci di elenco prezzi con gli estrattori roBERT per popolare le
                    proprietà mancanti.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <div className="flex-1">
                      <Select
                        value={selectedCommessaPropsId}
                        onValueChange={setSelectedCommessaPropsId}
