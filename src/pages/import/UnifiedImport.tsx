...
export default function UnifiedImport() {
  ...
  // Step 3: Mappatura colonne COMUNI
  ...
  // Colonne specifiche (prezzo/quantità per singolo, o globali per multi-impresa)
      ...
      if (kind === "multi-impresa") {
        if (mode === "lc") {
          if (!codeColumns.length) {
            throw new Error("In modalità LC devi selezionare almeno una colonna codice.");
          }
          if (!descriptionColumns.length) {
            throw new Error("In modalità LC devi selezionare almeno una colonna descrizione.");
          }
          if (progressiveColumn) {
            // il progressivo non è usato in LC
            setProgressiveColumn("");
          }
        }
        if (mode === "mc" && !progressiveColumn) {
            throw new Error("In modalità MC devi selezionare la colonna progressivo.");
        }
        const config = multiConfig
          .filter((c) => c.impresa.trim())
          .map((c) => ({
            nome_impresa: c.impresa.trim(),
            colonna_prezzo: mode === "lc" ? c.priceColumn || priceColumn : c.priceColumn || priceColumn || undefined,
            colonna_quantita: c.quantityColumn || quantityColumn || undefined,
        ...
        if (mode === "lc") {
          const missingPrice = config.some((c) => !c.colonna_prezzo);
          if (missingPrice) {
            throw new Error("Seleziona la colonna prezzo per tutte le imprese o scegli la modalità MC.");
          }
        }
        ...
        await api.uploadRitorniBatchSingleFile(commessaId, {
          ...
          progressiveColumn: mode === "lc" ? undefined : progressiveColumn || undefined,
        });
      } else {
        if (!impresa.trim()) {
        ...
        if (mode === "lc" && !priceColumn) {
          throw new Error("Seleziona la colonna prezzo");
        }
        if (mode === "lc" && !codeColumns.length) {
          throw new Error("In modalità LC devi selezionare la colonna codice.");
        }
        if (mode === "lc" && !descriptionColumns.length) {
          throw new Error("In modalità LC devi selezionare la colonna descrizione.");
        }
        if (mode === "mc" && !progressiveColumn) {
          throw new Error("In modalità MC devi selezionare la colonna progressivo.");
        }
        ...
        await api.uploadRitorno(commessaId, {
          ...
          progressColumn: mode === "lc" ? undefined : progressiveColumn || undefined,
        });
      }
  ...
  const canImport =
    uploadedFile &&
    (kind === "multi-impresa" || (impresa.trim() && priceColumn)) &&
        ...
        <CardContent className="space-y-6">
          ...
          {uploadedFile && headers.length > 0 && (
            <>
              ...
              <div className="space-y-4">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Colonne comuni</CardTitle>
                    <CardDescription>Queste colonne sono usate per tutte le imprese</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <div className="space-y-2">
                        <Label>Colonne codice {mode === "lc" && "(opzionale)"}</Label>
                        <p className="text-xs text-muted-foreground">Puoi selezionare più colonne</p>
                        <div className="space-y-2">
                          {codeColumns.map((col, idx) => (
                            <div key={idx} className="flex gap-2">
                              ...
                              <Button variant="ghost" size="icon" onClick={() => setCodeColumns(codeColumns.filter((_, i) => i !== idx))}>
                                ×
                              </Button>
                            </div>
                          ))}
                          <Button variant="outline" size="sm" onClick={() => setCodeColumns([...codeColumns, ""])}>
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {mode === "mc" ? 'Serve per trovare "TOTALE"' : "Puoi selezionare più colonne"}
                        </p>
                        <div className="space-y-2">
                          ...
                          <Button variant="outline" size="sm" onClick={() => setDescriptionColumns([...descriptionColumns, ""])}>
                ...
                {/* MULTI-IMPRESA: Tabella con prezzo/quantità per impresa */}
                {kind === "multi-impresa" && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Configurazione imprese</CardTitle>
                      <CardDescription>Definisci nome impresa e colonne prezzo/quantità per ciascuna</CardDescription>
                    </CardHeader>
                      ...
                      {multiConfig.map((row) => (
                        <div key={row.id} className="grid grid-cols-5 gap-2 items-end">
                          <Input
                            placeholder="Impresa"
                            value={row.impresa}
                            onChange={(e) => setMultiConfig((prev) => prev.map((r) => (r.id === row.id ? { ...r, impresa: e.target.value } : r)))}
                          />
                          <Select
                            value={row.priceColumn}
                            onValueChange={(val) => setMultiConfig((prev) => prev.map((r) => (r.id === row.id ? { ...r, priceColumn: val } : r)))}
                          >
                            <SelectTrigger>
                          ...
                          <Select
                            ...
                            onValueChange={(val) => setMultiConfig((prev) => prev.map((r) => (r.id === row.id ? { ...r, quantityColumn: val } : r)))}
                          >
                            <SelectTrigger>
                          ...
                          <Select
                            value={row.roundMode}
                            onValueChange={(val) =>
                              setMultiConfig((prev) => prev.map((r) => (r.id === row.id ? { ...r, roundMode: val as "auto" | "new" | "replace" } : r)))
                            }
                          >
                            <SelectTrigger>
                              <SelectValue />
                          ...
                          <Input
                            ...
                            onChange={(e) => setMultiConfig((prev) => prev.map((r) => (r.id === row.id ? { ...r, roundNumber: e.target.value } : r)))}
                ...
                {/* FILE SINGOLO: Solo prezzo e quantità */}
                {kind === "single" && (
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <Label>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-2">
              ...
              <Badge variant="secondary">{kind === "multi-impresa" ? "Multi-impresa" : "Singolo"}</Badge>
            </div>
