...
from ..llm.pipeline import LLMExtractionConfig, LLMPropertyExtractor, RuleCandidateGenerator
from ..llm.ollama_client import DEFAULT_MODEL as DEFAULT_OLLAMA_MODEL, DEFAULT_OLLAMA_URL, OllamaClient, OllamaConfig
...
def _extract_with_llm(
    ...
    *,
    ...
    schema_registry_path: Path,
    use_rule_candidates: bool,
    cache_size: int,
    fail_fast: bool,
    logger,
    trace_id: str,
):
    """Sync LLM extraction using the Ollama orchestrator."""

    config = LLMExtractionConfig(
        model=llm_model or DEFAULT_OLLAMA_MODEL,
        endpoint=llm_endpoint or DEFAULT_OLLAMA_URL,
        timeout=llm_timeout,
        registry_path=str(schema_registry_path),
        use_rule_candidates=use_rule_candidates,
        cache_size=cache_size,
        temperature=llm_temperature,
    )
    client = OllamaClient(OllamaConfig(base_url=config.endpoint, timeout=config.timeout))
    candidate_generator = RuleCandidateGenerator(config.registry_path) if use_rule_candidates else None
    extractor = LLMPropertyExtractor(llm_client=client, config=config, candidate_generator=candidate_generator)

    results = []
    with tqdm(total=len(records), desc="Extracting properties (LLM)", unit="doc") as pbar:
        for idx, record in enumerate(records):
            try:
...
def extract_properties(
    ...
    engine: Engine = typer.Option(
        ...
        "--engine",
        case_sensitive=False,
        help="Extraction engine: hybrid (regole+QA) o llm (Ollama compatto)",
    ),
    llm_use_rules: bool = typer.Option(
        True,
        "--llm-use-rules/--llm-no-rules",
        help="Precompila suggerimenti dai parser/matcher prima del prompt LLM",
    ),
    llm_cache_size: int = typer.Option(
        128,
        "--llm-cache-size",
        min=0,
        help="Cache LLM per evitare richieste duplicate (0 per disabilitare)",
    ),
    llm_temperature: float = typer.Option(
        0.0, "--llm-temperature", min=0.0, max=1.0, help="Temperatura passata al modello Ollama"
    ),
) -> None:
    ...
    with input_path.open("r", encoding="utf-8") as src:
        for line in src:
            ...
            record_category = record.get("categoria") or record.get("category") or record.get("cat")
    ...
    if engine == Engine.HYBRID and use_qa:
    ...
    if engine == Engine.LLM:
        results = _extract_with_llm(
            ...
            use_rule_candidates=llm_use_rules,
            cache_size=llm_cache_size,
            fail_fast=fail_fast,
            logger=logger,
            trace_id=trace_id,
        )
